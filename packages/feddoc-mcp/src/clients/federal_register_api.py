...
Handles all interactions with the Federal Register API including caching,
...
class FederalRegisterClient:
    ...
    Provides methods for searching regulations, accessing documents,
    and retrieving Federal Register data.
    """
    ...
    def __init__(self, cache_ttl: int = 300) -> None:
        ...
        # Set up rate limiter (1000 req/hour - conservative estimate)
        ...
        logger.info(f"Initialized FederalRegisterClient with base URL: {self.base_url}")
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        return {
            ...
            "User-Agent": "FedDocMCP/0.2.0 (github.com/n8daniels/FedDocMCP)",
        }
    ...
    async def _make_request(
        ...
        try:
            ...
            get_monitor().record_api_call("federal_register")
    ...
    async def search_documents(
        ...
        Args:
            ...
            agency: Agency identifier or name
        ...
        # Build params
        params: Dict[str, Any] = {
            "conditions[term]": query.strip(),
            "per_page": limit,
            "page": (offset // limit) + 1,  # Federal Register uses page numbers
            "fields[]": [
                "title",
                "type",
                "abstract",
        ...
        if filter_start_date:
            params["conditions[publication_date][gte]"] = filter_start_date
        if filter_end_date:
            params["conditions[publication_date][lte]"] = filter_end_date

        # Make request
        log_msg = f"Searching Federal Register: query='{query}', limit={limit}"
        if filter_start_date and filter_end_date:
            log_msg += f", date_range={filter_start_date} to {filter_end_date}"
        logger.info(log_msg)

        response = await self._make_request("/documents", params)
    ...
    async def get_document_details(self, document_number: str) -> Dict[str, Any]:
        """
        Get detailed information about a specific Federal Register document.
        ...
        # Build endpoint
        endpoint = f"/documents/{document_number.strip()}"
    ...
    async def get_public_inspection_documents(
        self,
        agency: Optional[str] = None,
        special_filing: bool = False,
    ) -> List[Dict[str, Any]]:
        """
        Get documents on public inspection (filed but not yet published).

        Args:
            agency: Filter by agency identifier
            special_filing: Include only special filings (default: False)

        Returns:
            List of public inspection documents
        ...
        if special_filing:
            params["conditions[special_filing]"] = 1

        # Make request
        logger.info("Getting public inspection documents")
        response = await self._make_request("/public-inspection-documents", params)

        # Extract documents from response
        documents = response.get("results", [])
        logger.info(f"Found {len(documents)} public inspection documents")
    ...
    def close(self) -> None:
        ...
        self.session.close()
        logger.info("Closed FederalRegisterClient session")
